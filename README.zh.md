...
9. **可选配置：**连接 USB 后打开 <https://openbrt.github.io/voxstick/config.html>，修改七个动作、平放闭麦、姿态阈值、切换保持时间、共享长按阈值、小狗形象和 Copilot 完成闪烁轮数
...
形象可选像素小狗（默认）、柴犬、柯基、拉布拉多或边牧；
[预览页](https://openbrt.github.io/voxstick/dog-preview.html)可同时查看五种效果。
旧版配置升级后默认保留像素小狗，不会改动已保存的动作或姿态参数。
...
本仓库通过 [`.github/hooks/voxstick-complete.json`](.github/hooks/voxstick-complete.json)
注册 VS Code Agent `Stop` hook。Agent 任务结束时，USB 连接的 voxstick 会全屏
红绿交替闪烁，结束后恢复最新的小狗或狗屋画面。默认闪 3 轮；网页配置范围为
`0–10`，其中 `0` 表示关闭。

该 hook 只在 voxstick 仓库生效，并且只走 USB 数据连接，不走 BLE。设备未连接、
接口忙、使用纯充电线或固件过旧时都会静默跳过，不影响 Copilot。原生 helper 会
按需编译，需要 `clang`、`pkg-config` 和 `libusb-1.0`；macOS 可运行
`brew install pkg-config libusb`，Linux 安装对应发行版软件包即可。Windows hook
目前是静默 no-op。VS Code Agent Hooks 仍是 Preview 功能，未来配置格式可能变化。
...
- ✅ LCD 状态显示，支持五种可选小狗形象
- ✅ 网页配置页（七动作、平放闭麦、姿态阈值、切换保持时间、共享长按阈值、小狗形象、完成闪烁轮数）
- ✅ VS Code Copilot Agent 完成后通过 USB 触发红绿闪烁
