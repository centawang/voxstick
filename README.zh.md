...
把麦克风和按键塞进 USB 棒里。**按一下说话，再按一下停，双击发送，长按退格**。
...
[USB-C] → 棒 → Mac
         ...
         └ 蓝牙级别的真键盘 (USB HID)：左 Ctrl+F12 / Enter / Backspace
```
...
- **轻按棒上的大按键** = 发 `左 Ctrl+F12` HID 报告 → 微信输入法的"免提模式"绑这个键 → 进语音状态
- 边说话边看微信输入法的浮窗实时上字 / 智能纠错
- **再轻按一下** = 第二次发 `左 Ctrl+F12` → 微信输入法停录、把最终文本插进输入框
- **双击棒上的大按键** = 发 `Enter` → 把消息发出去
- **长按棒上的大按键** ≥ 0.6 秒 = 发 `Backspace` → 删除前一个字符或当前选区
- **按一下 BtnB** = 发一次 `向下方向键` → 光标或当前选项向下移动一次
- **双击 BtnB** = 发一次 `向上方向键` → 光标或当前选项向上移动一次
- **摇晃一下棒** = 按下并释放一次 `左 Ctrl`
...
1. **设置 → 语音输入 → 免提模式 → 设置快捷键** → 按一下棒上的大按键 → 输入框捕获 `Ctrl+F12`（固件实际发左 Ctrl，macOS 可能显示为 `⌃F12`），保存
2. **系统设置 → 隐私与安全 → 辅助功能** → 打开"微信输入法"开关（首次需要授权）
3. **任意聊天/编辑器** → 把光标点进文本框 → 轻按棒说话 → 再轻按结束 → 双击发送
...
8. **可选配置：**打开 <https://openbrt.github.io/voxstick/config.html>，修改平放闭麦、轻按触发键和长按触发键
...
| Mac 工具 | 触发 | 备注 |
|---|---|---|
| **微信输入法** ⭐ | 左 Ctrl+F12 | 中英混合 + LLM 纠错最强 |
...
- ✅ BtnA 单击 PTT、双击 Enter、长按 Backspace
- ✅ BtnB 单击向下、双击向上
- ✅ IMU 平放静音
- ✅ IMU 摇晃触发一次左 Ctrl
- ✅ LCD 状态显示
