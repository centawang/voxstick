...
[USB-C] → 棒 → Mac
         ...
         └ 蓝牙级别的真键盘 (USB HID)：Ctrl+F12 / Enter
```
...
- **轻按棒上的大按键** = 发 `Ctrl+F12` HID 报告 → 微信输入法的"免提模式"绑这个键 → 进语音状态
- 边说话边看微信输入法的浮窗实时上字 / 智能纠错
- **再轻按一下** = 第二次发 `Ctrl+F12` → 微信输入法停录、把最终文本插进输入框
...
1. **设置 → 语音输入 → 免提模式 → 设置快捷键** → 按一下棒上的大按键 → 输入框捕获 `Ctrl+F12`（macOS 可能显示为 `⌃F12`），保存
...
7. **微信输入法：**进入 **设置 → 语音输入 → 免提模式 → 设置快捷键**，按一下棒上的大按键，绑定 `Ctrl+F12`（macOS 可能显示为 `⌃F12`）
