...
static vox_config_v4_wire_t config_v4_from_current(vox_config_wire_t const *cfg)
{
    return (vox_config_v4_wire_t) {
        .flat_mute_enabled = cfg->flat_mute_enabled,
        .reserved = 0,
        .btn_a_single = cfg->btn_a_single,
        .btn_a_double = cfg->btn_a_double,
...
static vox_config_v5_wire_t config_v5_from_current(vox_config_wire_t const *cfg)
{
    return (vox_config_v5_wire_t) {
        .flat_mute_enabled = cfg->flat_mute_enabled,
        .reserved = 0,
        .btn_a_single = cfg->btn_a_single,
        .btn_a_double = cfg->btn_a_double,
...
static vox_config_v6_wire_t config_v6_from_current(vox_config_wire_t const *cfg)
{
    return (vox_config_v6_wire_t) {
        .flat_mute_enabled = cfg->flat_mute_enabled,
        .reserved = 0,
        .btn_a_single = cfg->btn_a_single,
        .btn_a_double = cfg->btn_a_double,
                                  ...
                                  vox_config_wire_t const *cfg)
{
    ...
    if (status == VOX_CONFIG_STATUS_OK && cfg != NULL) {
        if (protocol_version == VOX_CONFIG_PROTOCOL_VERSION_V1) {
            ...
            payload_len = sizeof(legacy);
            memcpy(resp + CFG_HEADER_LEN, &legacy, payload_len);
        } else if (protocol_version == VOX_CONFIG_PROTOCOL_VERSION_V4) {
            vox_config_v4_wire_t legacy = config_v4_from_current(cfg);
            payload_len = sizeof(legacy);
            memcpy(resp + CFG_HEADER_LEN, &legacy, payload_len);
        } else if (protocol_version == VOX_CONFIG_PROTOCOL_VERSION_V5) {
            vox_config_v5_wire_t legacy = config_v5_from_current(cfg);
            payload_len = sizeof(legacy);
            memcpy(resp + CFG_HEADER_LEN, &legacy, payload_len);
        } else if (protocol_version == VOX_CONFIG_PROTOCOL_VERSION_V6) {
                                 ...
                                 uint16_t bufsize)
{
    ...
    case VOX_CONFIG_CMD_SET:
        if (version == VOX_CONFIG_PROTOCOL_VERSION_V1) {
            ...
            cfg.long_press_ms = legacy.long_press_ms;
        } else if (version == VOX_CONFIG_PROTOCOL_VERSION_V4) {
            if (payload_len != sizeof(vox_config_v4_wire_t)) {
                write_config_response(itf, version,
                                      VOX_CONFIG_STATUS_BAD_REQ, NULL);
                return true;
            }
            vox_config_v4_wire_t legacy = {0};
            memcpy(&legacy, buffer + CFG_HEADER_LEN, sizeof(legacy));
            vox_config_get(&cfg);
            memcpy(&cfg, &legacy, sizeof(legacy));
        } else if (version == VOX_CONFIG_PROTOCOL_VERSION_V5) {
            if (payload_len != sizeof(vox_config_v5_wire_t)) {
                write_config_response(itf, version,
                                      VOX_CONFIG_STATUS_BAD_REQ, NULL);
                return true;
            }
            vox_config_v5_wire_t legacy = {0};
            memcpy(&legacy, buffer + CFG_HEADER_LEN, sizeof(legacy));
            vox_config_get(&cfg);
            memcpy(&cfg, &legacy, sizeof(legacy));
        } else if (version == VOX_CONFIG_PROTOCOL_VERSION_V6) {
            if (payload_len != sizeof(vox_config_v6_wire_t)) {
                write_config_response(itf, version,
                                      VOX_CONFIG_STATUS_BAD_REQ, NULL);
                return true;
            }
            vox_config_v6_wire_t legacy = {0};
            memcpy(&legacy, buffer + CFG_HEADER_LEN, sizeof(legacy));
            vox_config_get(&cfg);
            memcpy(&cfg, &legacy, sizeof(legacy));
